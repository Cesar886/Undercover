...
export function AuthorMenu({ onEdit, onDelete, onToggleHidden, hidden = false, size = 'md' }: AuthorMenuProps) {
  ...
  return (
    <div ref={ref} className="relative inline-block">
      <button ref={triggerRef} type="button" onClick={() => setOpen((value) => !value)}
        ...
        aria-label="Más opciones" aria-haspopup="menu" aria-expanded={open}>
        <MoreHorizontal size={iconSize} strokeWidth={1.8} />
      </button>
      {open && (
        <div role="menu" className="absolute right-0 mt-1 w-40 bg-white dark:bg-[#0d0b1a] border border-stone-200 dark:border-violet-500/20 rounded-lg shadow-lg dark:shadow-[0_8px_24px_rgba(124,58,237,0.15)] z-20 overflow-hidden animate-fade-in">
          {onToggleHidden && (
            <button role="menuitem" onClick={() => { setOpen(false); onToggleHidden(); }}
