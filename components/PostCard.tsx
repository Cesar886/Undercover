...
export function PostCard({
  ...
  async function handleShareSticker(e: React.MouseEvent) {
    e.preventDefault();
    e.stopPropagation();
    if (!post.image_webp) return;
    try {
      const res = await fetch(post.image_webp);
      ...
      if (navigator.share && navigator.canShare && navigator.canShare({ files: [file] })) {
        ...
        alert('Tu navegador no soporta enviar archivos directamente.');
      }
    } catch (err) {
      console.error('Error compartiendo sticker:', err);
    }
  }
          ...
          <div className="flex items-center gap-3">
            ...
            {post.category === 'stickers' && post.image_webp ? (
              <button
                ...
                className="flex items-center gap-1.5 text-stone-300 dark:text-[#2e2b4a] hover:text-[#25D366] dark:hover:text-[#25D366] transition-colors relative z-[10]"
                title="Añadir Sticker a WhatsApp"
                aria-label="Añadir a WhatsApp"
              >
                ...
                <span className="text-[11px] font-medium hidden sm:inline">Añadir</span>
              </button>
            ) : (
              <a
