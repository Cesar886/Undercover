...
export function PostCard({
  ...
  function handleDownloadSticker(e: React.MouseEvent) {
    e.preventDefault();
    e.stopPropagation();
    if (!post.image_webp) return;
    const a = document.createElement('a');
    a.href = post.image_webp;
    a.download = 'sticker-um.webp';
          ...
          <div className="flex items-center gap-3">
            ...
            {post.category === 'stickers' && post.image_webp ? (
              <button
                ...
                className="flex items-center gap-1.5 text-stone-300 dark:text-[#2e2b4a] hover:text-yellow-500 dark:hover:text-yellow-400 transition-colors relative z-[10] touch-manipulation"
                title="Descargar sticker (.webp)"
                aria-label="Descargar sticker"
              >
